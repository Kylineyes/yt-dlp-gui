...
- 所有 SQLite schema、查询、写入、迁移和测试 SQL 必须遵守根目录 [`sql-style.md`](sql-style.md)；该文件是 SQL 格式、参数化、事务、UPSERT 与 schema 约束的唯一规范来源。
- 程序启动时支持 `-c <路径>` 参数以选择配置数据库文件。默认路径须在后续实现中明确，不得在当前骨架阶段推断。
- 初始数据库仅包含空的 `config` 表；首次启动创建数据库和表结构后不写入配置数据，具体配置由后续配置流程写入。未确认需求前，不要预置任务、下载历史或 UI 偏好字段。
